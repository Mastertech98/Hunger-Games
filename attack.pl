...
kill_all_enemies(Length) :- get_all_nearby_enemy_length(Length), !,
...
is_enemy_nearby :- (
	current_position(X,Y), enemy_at(X,Y,_,_) -> !;
	write('Theres no enemy nearby...'), fail
).
...
has_weapon :- (
	get_weapon(Weapon), Weapon \== none -> !;
	write('You got no weapon tho'), nl, fail
).
...
attack :- (
	is_enemy_nearby, has_weapon -> !, write('attacking enemy nearby '), nl,
								   take_damage, !, nl, kill_all_enemies(R),
		                           write('You kill '), write(R), write(' enemy'), nl,
		          				   random_move_all_enemies,
		  						   check_game_over, check_win
